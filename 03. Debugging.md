...
`Chez Scheme` 有几个特性可以支持调试。除了对完全类型检查过的代码提供错误信息之外，`Chez Scheme` 也支持对过程调用进行跟踪，中断任意计算，对异常和中断处理器的重定义，检查任意对象，包括异常的Continuation 和中断。

`Scheme` 或者 `Chez Scheme` 的新手程序员，甚至是有经验的 Scheme 程序员都可以参考“How to Debug Chez Scheme Programs” 这个教程。在 <http://www.cs.indiana.edu/chezscheme/debug/> 可以找到 HTML 和 PDF 版本的教程。(译者注：此链接在翻译时无法打开 2019/2/1)

## 3.1 跟踪

跟踪是调试 Scheme 程序最有用的手段之一。 `Chez Scheme` 可以跟踪任何内建和用户定义的过程。跟踪的包会将每一个调用过程的参数和返回值打印出来，通过紧凑的缩进机制来显示嵌套调用的深度。打印非尾递归调用的时候会增加缩进而尾递归调用不会，可以以此区分是尾递归调用还是非尾递归调用。对于嵌套大于等于 10 的调用，会在缩进处有一个括号，其中包括一个数字用于说明嵌套的深度。
...
一个 `trace-lambda` 表达式等价于一个 `lambda` 表达式加上一样的语法形式以及程序体，除了无论何时该过程被调用的时候都会向跟踪端口输出跟踪信息。这里使用 `name` 来标识该过程。跟踪信息会显示过程的参数以及过程的返回值，嵌套的调用会以缩进显示。
...
以上的例子展示了跟踪的时候对于尾递归和非尾递归调用的不同处理方式。因为 `half` 在参数是奇数的时候会调用自身,所以 `(half 5)` 与 `(half 4)` 的缩进层级是一样的，另外，因为 `(half 5)` 和 `(half 4)`的返回值是一样的所以这两个调用的返回值只显示一个。
...
检查器可以直接通过调用过程 `inspect` 或者从调试器间接启动。它允许程序员检查循环对象、端口和过程这类没有 reader 语法的对象、continuation 对象、程序员无法直接接触到的变量以及通常的可以被打印的对象。

检查器的首要目的是检查而不是修改对象。但是，可被赋值的变量的值有可能在检查器内被改变。可被赋值的变量通常被限制于在源程序中出现的变量。也可以对于一个对象调用任意的过程（包括一些修改变量的过程例如 `set-car!` ）那些原本就是不可改变的对象是不可以被改变的，例如不可被赋值的变量、过程以及大数（bignum），因为这么做违反了编译器和运行时的假定。

用户在使用时可以看到包括当前对象的打印展示的提示行，如果有必要的话会使用缩写让一行可以放得下。大量的命令可以用来展示当前的对象以及在对象内游走。检查器还提供了可用选项的“在线”描述。命令"?"会显示可以应用于当前对象的可用命令。命令"??" 会显示通用的命令。命令"h"会给出如何使用检查器的简要描述。EOF 字符或者命令"q"可以退出检查器。

-   过程：	(inspect obj)
-   返回值：	不确定
-   所属库：	(chezscheme)

如上所述，在 `obj` 上调用检查器。依据当前的对象，不同的检查器可用命令罗列如下：

**通用命令**

`help` 或 `h` 会显示关于如何使用检查器的简要描述
...
`goto` 或 `g m` 回到标记着 `m` 的位置。如果 `m` 没有指定，检查器会回到默认标志标记的位置。
...
`show` 或 `s` 展示 `continuation`（下一个帧），如果有的话展示调用过程的源代码，待判定的调用源码，闭包，当前帧以及自由变量值。只有在编译对应的 lambda 表达式的时候启用了生成检查器信息时才可以得到源码。
...
对于 `/` 开头的绝对路径（或者 Windows 使用 `\` ），检查器会先尝试绝对路径，然后在设定的源代码文件夹下尝试路径的最后一部分（文件名）。对于 `./` (Windows 下是 `.\`) 或 `../` (Windows 下是 `..\` ) 开头的路径名,检查器首先检查 `.` 或 `..` ,然后尝试设定的源代码文件夹中 `.` 或 `..` 开头的路径，最后是在源代码文件夹中搜索路径的最后一部分（文件名）。对于其他的（相对的）路径名，检查器会先在设定的源代码目录中寻找整个相对路径，然后是在设定的源代码目录中寻找最后的部分（文件名）。

如果找到了名字相同但是内容与原本的源文件不同的文件，这个文件会被跳过。这通常是因为文件在编译之后又被
修改过。给定一个明确的文件名作为参数来强制打开特定文件（参考上面的通用命令）。

`eval` 或 `e expr` 在包含帧绑定的环境中对 `expr` 进行求值。在被求值的表达式内部，每一个帧 `n` 可以通过 `%n` 获取。同样的命令帧可以通过他们的名字得到。只有在编译对应的 lambda 表达式时启用了生成检查器信息时才可以得到名字。

`set!` 或 `!n e` 设置第 n 个帧元素为 `e` ，如果那个元素是可以被赋值的。 `n` 默认是 0。

**Procedure 命令（过程）**

`show` 或 `s` 展示源代码和过程中的自由变量。只有在编译对应的 lambda 表达式时启用了生成检查器信息时才可以得到源代码。
...
`eval` 或 `e expr` 在包含该过程的绑定的环境中对 `expr` 求值。在被求值的表达式内部，每一个自由变量 `n` 的值可以通过变量 `%n` 来获取。具名的变量可以通过他们的名字获取到变量值。只有在编译对应的 lambda 表达式的时候启用了生成检查器信息才可以得到变量名。   
...
通过组合不同的检查器的接口，系统还提供了非交互式的检查器。和交互式的检查器一样，也可以使用非交互式的方式来检查那些通常来说无法检查的对象。非交互式检查器遵循单一的非交互式的协议。一般的 Scheme 对象都是分装在过程或者检查器对象中的，这些过程或检查器对象接收一个符号性的信息然后返回关于被分装对象的信息或者是新的分装着对象一部分的检查器对象。

-   过程:	 (inspect/object object)
-   返回: 	一个检查器对象过程
-   所属库:	(chezscheme)

`inspect/object` 用于将一个 Scheme 对象转化为一个检查器对象。所有的检查器对象都接受 `type` 、 `print` 、`write` 和 `size` 这四个消息。 `type` 用于返回对象的类型的符号表示。 `print` 和 `write` 一定要和端口参数一起使用。这两条消息用于通过 Scheme 过程 `preety-print` 和 `write` 将对象的表示写到对应端口。 `size` 消息会返回当前对象所占用的字节大小的常数表示,包括任意当前对象可以访问的对象，但是除了已经使用同一个`inspect/object` 调用计算过大小的检查器对象。

除了变量检查器对象所有的检查器对象都接受消息 `value` ，这会返回当前检查器对象真正封装的对象。
...
**Pair（序对）检查器对象**  序对检查器对象包含一个 Scheme 的序对。

`(pair-object 'type)`   返回符号 `pair` 。

`(pair-object 'car)`    返回检查器对象中包含的序对的 `car` 数据成员。

`(pair-object 'cdr)`    返回检查器对象中包含的序对的 `cdr` 数据成员。

`(pair-object 'length)` 返回语句形式的列表 (与类型有关). 类型部分包含符号 `proper` 、 `omproper`
或是 `circular`，这会依据列表结构的不同而不同。计数部分包含该列表中有区别的序对的数量。

**Box 检查器对象** Box 检查器对象包含 Chez Scheme 的 box。

`(box-object 'type)`     返回符号 `box` 。
`(box-object 'unbox)`    返回包含 box 内容的检查器对象。

**TLC 检查器对象** Box 检查器对象包含 Chez Scheme 的 box。

`(tlc-object 'type)`     返回符号 `tlc` 。

`(tlc-object 'keyval)`   返回一个检查器对象,包含 TLC 的 keyval。

`(tlc-object 'tconc)`    返回一个检查器对象,包含 TLC 的 tconc。

`(tlc-object 'next)`     返回一个检查器对象,包含 TLC 的 下一个链接。

**Vector, String, Bytevector, 和 Fxvector 检查器对象** .向量（bytevector, string, fxvector）检查器对象包含 Scheme 的向量(bytevectors, strings, fxvectors)。

`(vector-object 'type)`   返回符号 `vector` ( `string`, `bytevector`, `fxvector`).

`(vector-object 'length)` 返回向量或字符串的元素数量。

`(vector-object 'ref n)`  返回一个检查器对象,包含向量或者字符串的第 `n` 个元素。

**简单检查器对象** ,简单检查器对象包含非结构化，不能修改的对象。包括数字、布尔值、空表、EOF 对象以及 void 对象。
可以直接通过给对象一个 `value` 参数获得。

`(simple-object 'type)`    返回符号 `symbol` 。

**未绑定检查器对象** ，虽然 Scheme 程序通常无法访问未绑定的对象，但在检查变量时可能会遇到它们。

`(unbound-object 'type)`   返回符号 `unbound`。

**过程检查器对象** 过程检查器对象包含一个 Scheme 过程。

`(procedure-object 'type)`   返回符号 `symbol` 。

`(procedure-object 'length)` 返回自由变量的数量 。

`(procedure-object 'ref n)`  返回一个检查器对象，包含过程的第 n 个自由变量。详见下面的变量检查器的部分。n必须是非负数且小于过程中自由变量的数量。

`(procedure-object 'eval expr)` 对 `expr` 求值并返回其值。过程中的自由变量在被求值的表达式中使用标识符
%n 表示，n 是检查器显示的位置数字。具名的变量同样也绑定到他们本身的名字上。

`(procedure-object 'code)`  返回一个检查器对象,其中包含过程的代码对象。关于代码检查器对象详见下文。

**continuation 检查器对象** 通过 `call/cc` 产生的 continuation 实际上是过程。但是在检查这样一个过程时，有可能会需要将 continuation 的底层数据结构暴露出来才行。一个 continuation 的数据结构包含要回复计算的位置，执行计算需要用到的变量以及下一个 continuation 的链接。

`(continuation-object 'type)`  返回符号 `continuation` 。

`(continuation-object 'length)` 返回自由变量的数量。

`(continuation-object 'ref n)`  返回一个检查器对象，包含 continuation 的第 n 个自由变量。详见下面的变量检查器的部分。n必须是非负数且小于过程中自由变量的数量。

`(continuation-object 'eval expr)` 对 `expr` 求值并返回其值。continuation 帧中的自由变量在被求值的表达式中使用标识符`%n` 表示，n 是检查器显示的位置数字。具名的变量同样也绑定到他们本身的名字上。

`(continuation-object 'code)` 返回一个检查器对象，返回过程的代码对象，在当前的 continuation 帧被创建时对应过程是正在运行的。详见下文有关代码检查器对象。

`(continuation-object 'depth)` 返回当前 continuation 包含的帧的数量。

`(continuation-object 'link)`  返回一个检查器对象，其中包含下一个 continuation 帧。continuation 的深度必须大于 1。

`(continuation-object 'link* n)` 返回一个检查器对象，其中包含 continuation 的第 n 个链接，n 必须小于深度。

`(continuation-object 'source)` 返回一个检查器对象，其中包含与 continuation 相关的源代码的信息（程序中导致 continuation 形成的源码）。如果没有附加源码信息那么返回 `#f` 。

`(continuation-object 'source-object)` 返回一个检查器对象，其中包含产生 continuation 形成相关的源码对象，如果没有附加源码对象那么返回 `#f` 。
...
**Code(代码) 检查器对象** 一个检查器对象包含 Chez Scheme 的代码对象

`(code-object 'type)`  返回一个符号 `code`

`(code-object 'name)`  返回一个字符串或者 `#f` 。代码检查器的名字就是对应过程原本绑定或者是被赋值时的名字。因为变量的绑定可以被改变所以这个名字可能会对不上。 如果检查器不能获悉过程的名字就返回 `#f` 。

`(code-object 'source)` 返回代码对象附加的源码信息，如果没有源码信息就返回 #f。

`(code-object 'source-object)`  返回一个检查器对象，其中包含代码对象的源对象，如果没有附加源对象则返回 #f。
...
**Variable（变量）检查器对象** 。 变量检查器对象封装了变量绑定。尽管真正的底层表示可能会不同，变量检查器对象提供了统一的交互界面。
...
`(port-object 'name)`    返回一个检查器对象包含端口的名字。

`(port-object 'handler)` 返回一个过程检查器对象，其中封装了端口的句柄，就像 `port-handler` 会返回的那样。
...
`(port-object 'output-buffer)` 返回一个检查器对象 ，其中包含用于输出缓冲的字符串。

`(port-object 'input-size)` 如果是一个输入端口的话返回一个输出缓冲区大小的常数，否则的话返回值是不确定的。

`(port-object 'input-index)` 如果是一个输入端口则返回一个输入缓冲区大小的常数，否则返回值不确定。

`(port-object 'input-buffer)`  返回一个检查器对象 ，其中包含用于输入缓冲的字符串。

**Symbol(符号)检查器对象** 。 符号检查器对象包含符号，包活 gensyms（生成符号）

`(symbol-object 'type)`  返回符号 `symbol`。

`(symbol-object 'name)`  返回一个字符串检查器对象。一个符号检查器的的字符串表示的名字就是该符号的字符串表示。就像过程`symbol->string` 会返回的那样。

`(symbol-object 'gensym?)` 如果一个符号是生成符号则返回 `#t` ，否则返货 `#f` 。生成符号使用 `gensym` 生成。

`(symbol-object 'top-level-value)`  返回一个检查器对象 ，其中包含符号的全局值。

`(symbol-object 'property-list)` 返回一个检查器对象，其中包含符号的属性列表。

**Record(记录)检查器对象** 。记录检查器对象中包含记录。

`(record-object 'type)`  返回符号 `record`

`(record-object 'name)` 返回一个字符串检查器对象，其中包含记录类型的名字。

`(record-object 'fields)`  返回一个检查器对象，其中包括记录类型的成员名字列表。

`(record-object 'length)`  返回数据成员的数量。

`(record-object 'rtd)`   返回一个检查器对象，其中包含记录类型的记录类型描述符。

`(record-object 'accessible? name)` 如果对应名字的数据成员是可以访问的就返回 `#t` ,否则返回 `#f` 。如果编译器优化过数据成员有可能不能被访问。

`(record-object 'ref name)` 返回一个检查器对象，其中包括对应名字数据成员的值。如果对应数据成员不可访问则会抛出`&assertion` 异常。
...
交互式的检查器提供了 `find` 和 `find-next` 这些方便的方式来寻找对象。
