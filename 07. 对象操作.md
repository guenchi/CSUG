...
这些标识符是 `define-record` 的辅助关键字。除了在被识别为辅助关键字的上下文中，在其它任何地方引用这些标识符都是违反语法的。 `mutable` 和 `immutable` 也是 `define-record` 的辅助关键字，和 R<sup>6</sup>RS 报告中的 `define-record-type` 一样。
...
这个语法形式等价于 R<sup>6</sup>RS 报告中  `record-type-descriptor`  的语法形式。
