...
`define-property` 属于定义，可以出现在其他定义可以出现的任何地方。`define-property` 引入的属性的范围是 `define-property` 形式所出现的整个程序体，如果它出现在顶层，则是全局的，除非它由相同 `id` 和 `key` 的属性替换，或者它所附着的标识符的绑定被遮蔽（shadowed）了。可以使用不同的键将任意数量的属性附加到同一个绑定。将与已附加的属性同名的新属性附加到绑定上，会使新属性遮蔽现有属性。
