...
键列表之后的每个子句都包含一个模式 `pat`，一个可选 `fender`和 `template`。可选 `fender`经常被省略。该专利规定的语法输入表达式必须具有被选择该条。模式中不是关键字(模式变量)的标识符绑定到输入表达式的相应部分。`fender`(如果存在的话)是一个 Scheme表达式，它指定对输入表达式(通过模式变量访问)的附加约束，以便选择子句，必须满足这些约束。该模板指定何种形式的输出需要，通常在模式变量的子句。
